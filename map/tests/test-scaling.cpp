...
// Enables iteration test on a map larger than the memory available to the
// remote cluster
...
class StdMapWrapper {
 ...
 public:
  ...
  StdMapWrapper(StdMapWrapper &&other) : m_map(std::move(other.m_map)) {}

  StdMapWrapper(const StdMapWrapper &other) : m_map(other.m_map) {}
  ...
  ConstIterator cbegin() const { return m_map.begin(); }

  ReverseIterator rbegin() { return m_map.rbegin(); }
  ...
  */
  ...
  ConstIterator cend() const { return m_map.cend(); }

  ReverseIterator rend() { return m_map.rend(); }
  ...
  */
  ...
  ///////// Modifiers
  Iterator insert(const value_type &value) { return m_map.insert(value).first; }

  Iterator insert(value_type &&value) {
    return m_map.insert(std::move(value)).first;
  }

  void erase(const K &k) { m_map.erase(k); }
  ...
  ConstIterator find(const K &k) const { return m_map.find(k); }
 ...
 private:
  ...
  friend bool operator==(const StdMapWrapper<A, B> &,
                         const StdMapWrapper<A, B> &);
...
T ascendingInsert(int count, bool print = true) {
  ...
  if (print)
    std::cout << "Inserting " << count << " elements in aescending order took "
              << elapsed.count() << " milliseconds" << std::endl;
...
T descendingInsert(int count, bool print = true) {
  ...
  if (print)
    std::cout << "Inserting " << count << " elements in descending order took "
              << elapsed.count() << " milliseconds" << std::endl;
  return map;
}
...
void deleteTest() {
  ...
  start = system_clock::now();
  for (const int e : toDelete) m1.erase(e);
  end = system_clock::now();

  Milli elapsed1 = end - start;

  std::cout << "deleting 10000 elements from a map of size 10000 took "
            << elapsed1.count() << " milliseconds" << std::endl;
  ...
  start = system_clock::now();
  for (const int e : toDelete) m2.erase(e);
  end = system_clock::now();

  Milli elapsed2 = end - start;

  std::cout << "deleting 10000 elements from a map of size 100000 took "
            << elapsed2.count() << " milliseconds" << std::endl;
  ...
  start = system_clock::now();
  for (const int e : toDelete) m3.erase(e);
  end = system_clock::now();

  Milli elapsed3 = end - start;

  std::cout << "deleting 10000 elements from a map of size 1000000 took "
            << elapsed3.count() << " milliseconds" << std::endl;
  ...
  start = system_clock::now();
  for (const int e : toDelete) m4.erase(e);
  end = system_clock::now();

  Milli elapsed4 = end - start;

  std::cout << "deleting 10000 elements from a map of size 10000000 took "
            << elapsed4.count() << " milliseconds" << std::endl;
}
...
void findTest() {
  ...
  std::cout << "Finding 10000 elements from a map of size " << m1.size()
            << " took " << elapsed1.count() << " milliseconds" << std::endl;
  ...
  std::cout << "Finding 10000 elements from a map of size " << m2.size()
            << " took " << elapsed2.count() << " milliseconds" << std::endl;
  ...
  std::cout << "Finding 10000 elements from a map of size " << m3.size()
            << " took " << elapsed3.count() << " milliseconds" << std::endl;
  ...
  std::cout << "Finding 10000 elements from a map of size " << m4.size()
            << " took " << elapsed4.count() << " milliseconds" << std::endl;
}
...
void iterationTest(int count) {
  ...
  std::cout << "Iterating across " << count << " elements in a map of size "
            << count << " took " << elapsed.count()
            << " milliseconds time per iteration was "
            << elapsed.count() / double(count) * 1e6 << " nanoseconds"
            << std::endl;
}
...
void copyTest(int count) {
  ...
  std::cout << "Copy construction of a map of size " << m2.size() << " took "
            << elapsed.count() << " milliseconds" << std::endl;
}
...
int main() {
  ...
  /*
    Remember that some of these maps get quite large - iteration times may be
    affected by things other than the scaling of your algorithm.
  ...
  */
  {
    ...
    // Optional test. This is more ram than the remote machines have and will
